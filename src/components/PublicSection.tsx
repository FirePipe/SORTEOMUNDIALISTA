...
export default function PublicSection({ participants, eventState, appConfig }: PublicSectionProps) {
  ...
  return (
    <div className="relative min-h-[620px] bg-slate-50 dark:bg-[#020617] border border-slate-200 dark:border-blue-500/20 rounded-[2.5rem] p-8 flex flex-col justify-between overflow-hidden shadow-[0_0_80px_rgba(30,58,138,0.05)] dark:shadow-[0_0_80px_rgba(30,58,138,0.3)]">
      ...
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[550px] h-[550px] border border-blue-500/5 dark:border-blue-500/10 rounded-full blur-[1px] animate-[spin_20s_linear_infinite]" />
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] border border-amber-500/3 dark:border-amber-500/5 rounded-full blur-[2px] animate-[spin_35s_linear_infinite_reverse]" />
      
      {/* Laser glow lines */}
      <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-blue-500/20 dark:via-blue-500/50 to-transparent" />
      <div className="absolute bottom-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-amber-500/10 dark:via-amber-500/30 to-transparent" />

      {/* Header Spectator Bar */}
      <div className="relative z-10 flex flex-col sm:flex-row justify-between items-center pb-8 border-b border-slate-200 dark:border-white/10 gap-4">
        <motion.div 
        ...
        >
          ...
          <div>
            <h1 className="text-slate-800 dark:text-white text-lg font-black uppercase tracking-[0.2em] font-sans">
              {appConfig.nombreEvento}
            </h1>
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 bg-emerald-500 rounded-full animate-ping" />
              <p className="text-blue-600/70 dark:text-blue-400/60 text-[10px] font-bold uppercase tracking-widest flex items-center gap-2">
                Transmisión en Vivo • Auditado 
                <span className="ml-2 px-2 py-0.5 bg-blue-500/5 dark:bg-blue-500/10 rounded-full border border-blue-500/20 text-emerald-600 dark:text-emerald-400 text-[9px]">
                  PING: {ping}ms
                </span>
              </p>
            </div>
        ...
        >
          <div className="flex items-center gap-3 bg-slate-100 dark:bg-blue-950/40 border border-slate-200 dark:border-blue-500/20 px-4 py-2 rounded-2xl text-blue-700 dark:text-blue-300 text-sm shadow-xl backdrop-blur-sm">
            ...
            >
              <div className="space-y-4">
                <motion.div
                  ...
                  className="inline-flex items-center gap-2 px-4 py-1 rounded-full bg-blue-500/5 dark:bg-blue-500/10 border border-blue-500/10 dark:border-blue-500/20"
                >
                  <Activity className="w-3.5 h-3.5 text-blue-500 dark:text-blue-400 animate-pulse" />
                  <span className="text-blue-600 dark:text-blue-400 text-[10px] font-black uppercase tracking-[0.3em]">
                    {isRolling ? 'Procesando Aleatorización...' : 'Resultado Tentativo'}
                  </span>
                </motion.div>

                <h2 className="text-4xl md:text-6xl font-black text-slate-800 dark:text-white tracking-tighter leading-[1.1] max-w-2xl">
                  {displayParticipant?.nombre} {displayParticipant?.apellido}
                </h2>
                <div className="flex items-center justify-center gap-3 text-blue-650/70 dark:text-blue-400/70 font-bold uppercase tracking-[0.2em] text-xs">
              ...
              {showCelebration && (
                ...
                >
                  <div className="bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-amber-500/10 dark:from-amber-500/20 dark:via-amber-500/10 dark:to-amber-500/20 border border-amber-500/20 dark:border-amber-500/30 px-8 py-3 rounded-2xl shadow-2xl backdrop-blur-md">
                    <p className="text-amber-700 dark:text-amber-400 font-sans font-black text-xs uppercase tracking-[0.2em] flex items-center gap-3">
                      <Sparkles className="w-4 h-4 text-amber-600 dark:text-amber-300 animate-bounce" />
      ...
      <div className="relative z-10 border-t border-slate-200 dark:border-white/10 pt-8 grid grid-cols-1 sm:grid-cols-3 gap-8">
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }}>
          <span className="text-slate-400 dark:text-gray-500 text-[10px] uppercase font-bold tracking-widest block mb-2">Progreso General</span>
          <div className="flex items-baseline gap-2">
            <span className="text-slate-800 dark:text-white text-3xl font-black">{totalAssigned}</span>
            <span className="text-slate-400 dark:text-gray-500 text-xs font-bold">/ 52 PARTICIPANTES</span>
          </div>
          <div className="w-full bg-slate-200 dark:bg-slate-900/50 h-2 rounded-full mt-3 overflow-hidden border border-slate-200 dark:border-white/5 p-[1px]">
            <motion.div 
              initial={{ width: 0 }}
              animate={{ width: `${Math.min(100, (totalAssigned / 52) * 100)}%` }}
              className="bg-gradient-to-r from-blue-600 via-amber-400 to-amber-600 h-full rounded-full shadow-[0_0_10px_rgba(59,130,246,0.3)] dark:shadow-[0_0_10px_rgba(59,130,246,0.5)]"
            />
          </div>
        </motion.div>

        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.5 }} className="flex flex-col justify-center">
          <span className="text-slate-400 dark:text-gray-500 text-[10px] uppercase font-bold tracking-widest block mb-1">Estado de Seguridad</span>
          <span className="text-emerald-600 dark:text-emerald-400 text-sm font-black uppercase tracking-wider flex items-center gap-2">
            <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
            Encriptación de Extremo a Extremo
          </span>
          <span className="text-[10px] text-slate-400 dark:text-gray-500 font-mono mt-1">Fisher-Yates (Entropy Seed: 0x8A2)</span>
        </motion.div>

        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.6 }} className="flex items-center sm:justify-end">
          <div className="text-right text-[10px] text-slate-400 dark:text-gray-500 font-mono space-y-1.5 bg-slate-100 dark:bg-white/5 p-4 rounded-2xl border border-slate-200 dark:border-white/5">
            <p className="flex justify-between gap-4"><span>CLOUD NODE:</span> <span className="text-blue-600 dark:text-blue-400">AWS-EAST-1</span></p>
            <p className="flex justify-between gap-4"><span>DB STATUS:</span> <span className="text-emerald-600 dark:text-emerald-400">SYNCHRONIZED</span></p>
          </div>
        </motion.div>
      </div>
    </div>
