  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-[#070D19]/60 backdrop-blur-xl border border-blue-500/10 p-5 rounded-2xl flex items-center justify-between shadow-xl">
          <div className="space-y-1">
            <span className="text-gray-400 text-[10px] font-mono uppercase tracking-wider block">Estado del Servidor</span>
        ...
        <div className="bg-[#070D19]/60 backdrop-blur-xl border border-blue-500/10 p-5 rounded-2xl flex items-center justify-between shadow-xl">
          <div className="space-y-1">
            <span className="text-gray-400 text-[10px] font-mono uppercase tracking-wider block">Motor de Persistencia</span>
            <span className={`text-sm font-bold flex items-center gap-1.5 ${isDbLocal ? 'text-amber-400' : 'text-blue-400'}`}>
          ...
          <div className={`p-2.5 rounded-xl border ${isDbLocal ? 'bg-amber-500/10 border-amber-500/20' : 'bg-blue-500/10 border-blue-500/20'}`}>
            <Database className={`w-5 h-5 ${isDbLocal ? 'text-amber-400' : 'text-blue-400'}`} />
          </div>
        </div>

        <div className="bg-[#070D19]/60 backdrop-blur-xl border border-blue-500/10 p-5 rounded-2xl flex items-center justify-between shadow-xl">
          <div className="space-y-1">
            <span className="text-gray-400 text-[10px] font-mono uppercase tracking-wider block">Nivel de Seguridad</span>
      ...
      {/* Main Logs Table */}
      <div className="bg-[#070D19]/80 border border-blue-500/10 rounded-2xl overflow-hidden shadow-2xl relative">
        <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-blue-500/10 to-transparent" />
        
        <div className="flex items-center justify-between p-5 border-b border-white/5 bg-[#0B1528]/50">
          <div className="flex items-center gap-3">
            <Terminal className="w-5 h-5 text-amber-400" />
            <div>
              <h2 className="text-white text-sm font-bold uppercase tracking-wider font-sans">
                Historial de Auditoría de Seguridad
              </h2>
              <p className="text-gray-400 text-xs">Registro cronológico auditable e inmutable</p>
            </div>
          </div>
          ...
          {token && logs.length > 0 && (
            <button
              onClick={onClearLogs}
              className="bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 text-rose-400 text-xs font-bold px-3.5 py-1.5 rounded-xl flex items-center gap-1.5 cursor-pointer transition-colors"
            >
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5 text-xs text-gray-300">
              {logs.length === 0 ? (
                ...
                logs.map((log, idx) => (
                  <tr key={log._id || log.id || idx} className="hover:bg-blue-500/5 transition-colors">
                      ...
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold font-mono border
                        ${log.accion === 'INICIO' || log.accion === 'LOGIN'
                          ...
                          : log.accion === 'NUMERO_CONFIRMADO' || log.accion === 'ASIGNACION_AUTOMATICA'
                            ? 'bg-amber-500/15 border-amber-500/35 text-amber-400 shadow-[0_0_8px_rgba(230,194,128,0.15)]'
